...
Sachez que dans la plupart des restaurants, il est possible que vous mettiez la main à la pâte (muahaha) en la cuisant vous-même !", country: 'Japon', city: "Osaka", category_list: "Oeuf, Poisson, Viande", timetoeat_list: "Déjeuner/ Dîner", point: 50 )
okonomiyaki.remote_photo_url = url
okonomiyaki.save

url = 'https://res.cloudinary.com/geraldinefd/image/upload/v1575543971/ramen.jpg'
ramen = Specialty.new(name: 'Ramen', description: 'Plat préféré de Naruto, les ramen sont des nouilles servies dans un bouillon. Elles sont généralement accompagnées de légumes, viande (plutôt du porc) ou poisson. A savoir : il y a BEAUCOUP de versions de ramen au Japon. Elles varient selon les régions. ', country: 'Japon', category_list: "Nouilles, Pâtes, Soupe, Poisson, Viande", timetoeat_list: "Déjeuner/ Dîner", point: 20 )
ramen.remote_photo_url = url
ramen.save

url = 'https://res.cloudinary.com/geraldinefd/image/upload/v1575540383/takoyaki.jpg'
takoyaki = Specialty.new(name: 'Takoyaki', description: "Les takoyakis sont un élément phare de la street food japonaise et particulièrement d'Osaka. Ce sont des petites boules (genre balle de ping-pong), composées de chou et de morceaux de poulpe. Si tu n'es pas amateur de bêbêtes à tentacules, oublie ce qu'on vient d'écrire et teste quand même, tout est bien caché dans la pâte !", country: 'Japon', city: 'Osaka', category_list: "Fruits de mer, Poisson, Street Food", timetoeat_list: "Déjeuner/ Dîner, Goûter", point: 50)
takoyaki.remote_photo_url = url
takoyaki.save
url = 'https://res.cloudinary.com/geraldinefd/image/upload/v1575558165/sushi.jpg'
sushi = Specialty.new(name: 'Sushi', description: "Tu pensais connaître les sushis ? Grave erreur… Déjà tu risques de les trouver meilleurs ici (forcément). Ensuite, savais-tu qu'il existe des centaines de variétés de sushis ? Que déguster correctement un sushi c'est le manger avec les doigts ? Qu'il y a un ordre à respecter ?... On n'en dit pas plus car c'est sur place qu'on mesure les différences avec nos expériences occidentales.", country: "Japon", category_list: "Riz, Poisson", timetoeat_list: "Déjeuner/ Dîner", point: 10)
...
kobe = Specialty.new(name: 'Boeuf de Kobe', description: "Tu as sûrement déjà entendu parler du bœuf de Kobé ? Si non, sache que c'est la crème de la crème en matière de bœuf. On raconte même que les bêtes seraient massées et nourries à la bière. Evidemment, le prix suit aussi cette réputation de luxe. Il y a plusieurs classifications, à toi de choisir selon tes goûts... et ton budget.", country: 'Japon', region: 'Kansai', city: "Kobé", category_list: "Viande", timetoeat_list: "Déjeuner/ Dîner", point: 100)
...
inarizushi = Specialty.new(name: 'Inarizushi', description: "Les inarizushi sont constitués d'une poche de tofu frit sucré et remplie de riz vinaigré. Ils ont été inventés à Tokyo par un chef qui souhaitait les associer aux temples inari (dédiés à la déesse de la croissance du riz).", country: "Japon", region: "Kanto", city: "Tokyo", category_list: "Riz, Friture" , timetoeat_list: "Déjeuner/Dîner", point: 40)
...
fugu = Specialty.new(name: 'Fugu', description: "Tu es un food trotter intrépide ? Le fugu, aussi appelé poisson globe contient un poison mortel. Il doit être vidé et débarrassé de ses parties toxiques par un chef spécialisé et possédant une licence. Il est généralement dégusté en sashimi. Attention, c'est un mets plutôt cher.", country: "Japon", region: "Kansai", city: "Osaka", category_list: "Poisson" , timetoeat_list: "Déjeuner/Dîner", point: 500)
fugu.remote_photo_url = url
fugu.save

url = 'https://res.cloudinary.com/geraldinefd/image/upload/v1575825291/poulpy.jpg'
poulpy = Specialty.new(name: 'Sucette de poulpe', description: "Elle sont généralement laquées à la sauce soja avec... un oeuf de caille dans la tête. Elles se trouvent facilement sur les marchés d'Osaka.", country: "Japon", region: 'Kansai', city: "Osaka", category_list: "Snack" , timetoeat_list: "Goûter", point: 500)
poulpy.remote_photo_url = url
poulpy.save

url = 'https://res.cloudinary.com/geraldinefd/image/upload/v1575819209/kushikatsu.jpg'
kushikatsu = Specialty.new(name: 'Kushikatsu', description: "Des fritures typiques d'Osaka au boeuf, porc, poulet, fruits de mer ou légumes. Elles sont en général servies sur des batônnets avec de la sauce Worcestshire. C'était un plat populaire pendant la guerre et il reste un des emblèmes de la street food aujourd'hui.", country: "Japon", region: 'Kansai', city: "Osaka", category_list: "Viande, Fruits de mer, Street food, Friture" , timetoeat_list: "Déjeuner/Dîner", point: 60)
kushikatsu.remote_photo_url = url
kushikatsu.save

url = 'https://res.cloudinary.com/geraldinefd/image/upload/v1575825687/ochazuke.jpg'
ochazuke = Specialty.new(name: 'Ochazuke', description: "Un plat très simple qui se prépare en versant de l'eau chaude ou du thé vert sur un bol de riz. Il peut être agrémenté de sésame ou poisson. Attention, si on vous le propose à Kyoto c'est probablement parce que vous avez abusé de l'hospitalité de vos hôtes et qu'on vous demande poliment de partir.", country: "Japon", region: 'Kansai', city: "Kyoto", category_list: "Riz" , timetoeat_list: "Déjeuner/Dîner", point: 20)
...
game1 = Game.new(user: user1, specialty: poulpy)
game1.save
game2 = Game.new(user: user1, specialty: mochi)
game2.save
game3 = Game.new(user: user1, specialty: ramen)
game3.save
game4 = Game.new(user: user1, specialty: sushi)
game4.save
game4 = Game.new(user: user1, specialty: ochazuke)
game4.save
puts '...finished'
